...
type ResultStatusKind = 'matched' | 'liked' | 'approved' | 'approve';
...
function ResultsPageContent() {
  ...
  // Track which tags are actually applied (for color logic) - initialize from session storage if filters were applied
      ...
      if (wasApplied) {
        ...
        if (savedFilters) {
          try {
            ...
            console.error('Error parsing saved filters for appliedTags:', e);
          }
        }
      }
    }
  ...
  const fetchCompatibleUsers = async (page = 1, applyFilters = false, filtersOverride?: typeof filters) => {
    ...
    try {
      ...
      if (applyFilters) {
        ...
        if (hasRequiredTag) params.filter_required = true;
      ...
      const transformedProfiles: ResultProfile[] = response.results.map((item) => ({
        ...
        theirMissingRequired: Boolean(item.their_missing_required),
        ...
        {loading && profiles.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {sortedProfiles.slice(0, visibleCount).map((profile, index) => {
              ...
              // Use appliedTags (what was actually applied) not filters.tags (current UI state)
              // Show pending color if:
              // 1. Pending tag is applied (they haven't answered my required)
              // 2. Their Pending tag is applied (I haven't answered their required)
              // 3. OR requiredOnly is on AND this profile is missing required AND Required tag is NOT applied
              const hasPendingTagApplied = appliedTags.includes('Pending');
              ...
              const normalizedProfileTags = profile.tags.map(t => t.toLowerCase());
              const alreadyLikedOrMatched = profile.isMatched || normalizedProfileTags.includes('like');
              const isApprovedState = !alreadyLikedOrMatched && normalizedProfileTags.includes('approve');
              const statusKind: ResultStatusKind = profile.isMatched
                ? 'matched'
                : normalizedProfileTags.includes('like')
                  ? 'liked'
                  : normalizedProfileTags.includes('approve')
                    ? 'approved'
                    : 'approve';
              const statusLabel =
                statusKind === 'matched'
                  ...
                  <CardWithProgressBorder percentage={compatibilityScore} variant={cardVariant}>
                    ...
                    >
                      ...
                      <div className="absolute top-0 left-0 z-20">
                        ...
                        >
                          <ResultStatusGlyph status={statusKind} />
                        </button>
                      </div>
              ...
              <div className="w-full max-w-2xl">
                ...
                <div className="mb-8 rounded-2xl ring-1 ring-purple-200/70 bg-gradient-to-br from-purple-50/60 to-white p-5 shadow-sm">
                  ...
                  {pendingFilters.requiredOnly && (
                    <div className="mb-4">
                      <div className="inline-flex w-full min-w-0 items-center bg-white rounded-lg p-1.5 ring-1 ring-purple-200">
                        <button
                          type="button"
                      ...
                      {/* Captioned like the profile's My / Their Required picker. */}
